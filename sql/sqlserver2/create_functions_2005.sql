...
BEGIN	
	...
	*/
	...
	SELECT
		...
		CASE
			WHEN ISNULL(o.assessment_performed_in72hrs,0) = 1 THEN ''
			WHEN r2.description = 'noproblem' THEN ''
			WHEN o.assessment_performed = 1 THEN ''
...
BEGIN	
	...
	SELECT
		...
		CASE
		    WHEN ISNULL(o.assessment_performed, 0) = 0 AND r2.description = 'noproblem' THEN ''
...
BEGIN	
	...
	SELECT	
		...
		CASE			
		    WHEN ISNULL(o.communication_assessment_performed,0) = 0 AND ncr2.description = 'noproblem' THEN ''
		...
		CASE			
			...
			WHEN ncr.description = 'unwell' THEN 'MU'
		...
		CASE
			WHEN ISNULL(o.swallowing_assessment_performed_in72hrs,0) = 0 AND nsr2.description = 'passedswallowscreen' THEN '0'
		...
		CASE
			WHEN ISNULL(o.swallowing_assessment_performed_in72hrs,0) = 0 AND nsr2.description = 'passedswallowscreen' THEN ''		
...
BEGIN	
	...
	SELECT	
		...
		AS end_rehab_date,
		CASE
			WHEN pdc.discharged_to = 'died' THEN 'D'
			...
			WHEN pdc.discharged_to = 'intermediateCare' THEN 'TC'
		...
		CASE
			...
			WHEN pdc.discharged_to = 'otherHospital' THEN '' 
			WHEN pdst.Stroke_neurology_specific_ESD = 'Y' THEN 'SNS'
			WHEN pdst.Non_specialist_ESD = 'Y' THEN 'NS'
		...
		CASE
			WHEN LEN(ISNULL(patient.S7DeathDate,'')) > 0 OR patient.S7StrokeUnitDeath = 'Y'  THEN '' -- In case the patient died 
			WHEN pdc.discharged_to = 'otherHospital' THEN '' 
			WHEN ISNULL(pdc.documentation_post_discharge, 9) = 1 THEN 'Y'
			WHEN ISNULL(pdc.documentation_post_discharge, 9) = 0  THEN 'N'
			ELSE ''
		...
		CASE
			...
			WHEN pdc.documented_evidence = 'No' THEN 'N'
		...
		CASE
			WHEN ISNULL( pdc.ssnap_participation_consent, '') = 'yes'  THEN 'Y'
			WHEN ISNULL( pdc.ssnap_participation_consent, '') = 'no'  THEN 'N'
			WHEN ISNULL( pdc.ssnap_participation_consent, '') = 'not_asked' THEN 'NA'
			ELSE ''
